##############################################################################
# Whisper Subtitle Plugin for AviUtl2 - v2.9.36
...
#
# ビルド: .\whisper_subtitle_v2_9_36.ps1 [出力ディレクトリ]
# 要件: Visual Studio 2022, CMake 3.15+
# 出力: whisper_subtitle_v2_9_36.aux2 (既存の whisper_subtitle_v2_8_10.aux2 / whisper_subtitle.aux2 は上書きしない)
#
...
$projDir = "$d\aviutl2_dev\whisper_subtitle_plugin_v2_9_36"
$src = "$projDir\src"
$logFile = "$d\whisper_build_log_v2_9_36.txt"
$ErrorActionPreference = "Continue"
[Console]::OutputEncoding = [System.Text.Encoding]::UTF8

"" | Out-File $logFile -Encoding UTF8
"Whisper Subtitle v2.9.36 Build $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')" | Out-File $logFile -Append -Encoding UTF8

Write-Host ""
Write-Host "============================================" -ForegroundColor Cyan
Write-Host " Whisper Subtitle v2.9.36 - Build" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""
...
static void SetupThread(){
    ...
    if(python.empty()){
        ...
        SetStatusW(L"Ready (v2.9.36)"); // v2.8.10【I】: SetWindowTextW直接呼びを置き換え(環境タブ凍結対策)+バージョン更新
        SetProgress(0);
    ...
    SetStatusW(L"Ready (v2.9.36)"); // v2.8.10【I】: SetupThread正常終了時にg_statusSetupが取り残され「fugashi確認中...」等で凍結していたバグの本丸修正+バージョン更新
    SetProgress(0);
...
static void TranscribeThread(){
    ...
    {std::ofstream f(Utf8ToWide(GetPluginDir() + "\\whisper_debug.log"), std::ios::trunc); f << "=== Whisper Subtitle v2.9.36 ===\n";}
    SaveSettings();
    ...
    if(ffmpeg.empty()){
        MsgBox(g_wnd,
            ...
            "Error", MB_OK|MB_ICONERROR);
        SetStatus("Ready (v2.9.36)"); SetProgress(0); SetBusy(false); return;
    }
    std::string python = GetEffectivePython();
    if(python.empty()){
        MsgBox(g_wnd,
...
static void WriteTestLog(){
    ...
    f << "===== Whisper Subtitle v2.9.36 test log =====\n";
    f << "datetime  : " << dbuf << "\n";
...
extern "C" __declspec(dllexport) void __cdecl RegisterPlugin(HOST_APP_TABLE* host){
    ...
    g_status = CreateWindowExW(0, L"STATIC", L"Ready (v2.9.36)", WS_CHILD|WS_VISIBLE, SC(14), SC(y), SC(W-24), SC(54), g_wnd, 0, g_hInst, 0); g_tabSubCtrls.push_back(g_status);
    ...
    y += 18; // y=330
    g_statusSetup = CreateWindowExW(0, L"STATIC", L"Ready (v2.9.36)", WS_CHILD, SC(14), SC(y), SC(W-24), SC(20), g_wnd, 0, g_hInst, 0); g_tabSetupCtrls.push_back(g_statusSetup);
...
project(whisper_subtitle_v2_9_36 LANGUAGES CXX)
set(CMAKE_CXX_STANDARD 17)
add_library(whisper_subtitle_v2_9_36 SHARED src/whisper_subtitle.cpp)
target_compile_definitions(whisper_subtitle_v2_9_36 PRIVATE UNICODE _UNICODE)
if(MSVC)
    target_compile_options(whisper_subtitle_v2_9_36 PRIVATE /utf-8 /wd4828)
endif()
...
set_property(TARGET whisper_subtitle_v2_9_36 PROPERTY MSVC_RUNTIME_LIBRARY "MultiThreaded")
set_target_properties(whisper_subtitle_v2_9_36 PROPERTIES
    OUTPUT_NAME "whisper_subtitle_v2_9_36"
    SUFFIX ".aux2"
    PREFIX ""
    RUNTIME_OUTPUT_DIRECTORY_RELEASE "`${CMAKE_BINARY_DIR}/Release"
)
target_link_libraries(whisper_subtitle_v2_9_36 PRIVATE comctl32 shell32 comdlg32 ole32)
"@
...
try {
    ...
    # Find output
    $aux2 = Get-ChildItem -Path $buildDir -Recurse -Filter "whisper_subtitle_v2_9_36.aux2" | Select-Object -First 1
    if($aux2){
        ...
        $dest = Join-Path $destDir "whisper_subtitle_v2_9_36.aux2"
        Copy-Item $aux2.FullName $dest -Force
        Write-Host ""
        Write-Host "============================================" -ForegroundColor Green
        Write-Host " BUILD SUCCESS! v2.9.36" -ForegroundColor Green
        Write-Host "============================================" -ForegroundColor Green
