##############################################################################
# Whisper Subtitle Plugin for AviUtl2 - v2.8
#
# 機能:
...
#
# ビルド: .\whisper_subtitle_v2_8.ps1 [出力ディレクトリ]
...
"Whisper Subtitle v2.8 Build $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')" | Out-File $logFile -Append -Encoding UTF8

Write-Host ""
Write-Host "============================================" -ForegroundColor Cyan
Write-Host " Whisper Subtitle v2.8 - Build" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""

$cpp = @'
// Whisper Subtitle v2.8
...
def _run_faster_whisper(model_size, language, device, clips, model_dir, output_path, err_path, beam_size=5, temperature=0, batch=None):
    ...
    with open(err_path, "a", encoding="utf-8") as ef:
        ef.write(f"v2.8 params: no_prev_text={no_prev_text} word_ts={word_timestamps} rep_penalty={rep_penalty} batched={use_batched} hotwords={hotwords_text[:50]} prompt={prompt_text[:50]}\n")
    results = []
    filtered_count = 0
...
static void SetupThread(){
    ...
    if(python.empty()){
        ...
        SetWindowTextW(g_status, L"Ready (v2.8)");
    ...
    // v2.8: Check for distil/kotoba model cache (HuggingFace format)
    if(!modelExists && mName.find("kotoba") == 0){
        std::string hfCache = mDir + "\\models--kotoba-tech--kotoba-whisper-v2.0-faster";
        modelExists = FileExistsU(hfCache);
    }
    if(modelExists){
...
static bool RunFasterWhisper(int mi, int di, int bi, int beamSize, int li, float temp, bool noPrevText, bool wordTs, bool repPenalty, const std::string& promptText, const std::string& hotwordsText, bool batched){
    const char* mn[] = {"tiny","base","small","medium","large-v3","large-v3-turbo","kotoba-whisper"};
...
static void TranscribeThread(){
    ...
    {std::ofstream f(Utf8ToWide(GetPluginDir() + "\\whisper_debug.log"), std::ios::trunc); f << "=== Whisper Subtitle v2.8 ===\n";}
    SaveSettings();
    ...
    if(ffmpeg.empty()){
        MsgBox(g_wnd,
            ...
            "Error", MB_OK|MB_ICONERROR);
        SetStatus("Ready (v2.8)"); SetProgress(0); g_busy = false; return;
    }
    std::string python = GetEffectivePython();
    if(python.empty()){
        MsgBox(g_wnd,
    ...
    if(!RunFasterWhisper(mi, di, bi, beamSize, li, temp, noPrevText, wordTs, repPenalty, promptText, hotwordsText, batched)){
        SetStatus("Ready (v2.8)"); SetProgress(0); g_busy = false; return;
    }
...
extern "C" __declspec(dllexport) void __cdecl RegisterPlugin(HOST_APP_TABLE* host){
    host->set_plugin_information(L"Whisper Subtitle v2.8");
    InitCommonControls();
    ...
    g_wnd = CreateWindowExW(0, L"WhisperSub27", L"Whisper", WS_POPUP, 0, 0, 360, 400, 0, 0, g_hInst, 0);
    int W = 340;
    HWND hw;

    // === Tab control (4 tabs) ===
    g_tab = CreateWindowExW(0, WC_TABCONTROLW, L"", WS_VISIBLE|WS_CHILD|WS_CLIPSIBLINGS, 4, 2, W+6, 360, g_wnd, 0, g_hInst, 0);
    ...
    // ================================================================
    int y = tabY + 8;
    hw = CreateWindowExW(0, L"STATIC", L"Model:", WS_CHILD|WS_VISIBLE, 14, y+3, 42, 18, g_wnd, 0, g_hInst, 0); g_tabSubCtrls.push_back(hw);
    g_modelCombo = CreateWindowExW(0, L"COMBOBOX", L"", WS_CHILD|WS_VISIBLE|CBS_DROPDOWNLIST|WS_VSCROLL, 58, y, 146, 120, g_wnd, 0, g_hInst, 0); g_tabSubCtrls.push_back(g_modelCombo);
    ...
    hw = CreateWindowExW(0, L"STATIC", L"\x8a00\x8a9e:", WS_CHILD|WS_VISIBLE, 212, y+3, 36, 18, g_wnd, 0, g_hInst, 0); g_tabSubCtrls.push_back(hw);
    g_langCombo = CreateWindowExW(0, L"COMBOBOX", L"", WS_CHILD|WS_VISIBLE|CBS_DROPDOWNLIST|WS_VSCROLL, 250, y, 90, 200, g_wnd, 0, g_hInst, 0); g_tabSubCtrls.push_back(g_langCombo);
    ...
    y += 30;
    hw = CreateWindowExW(0, L"BUTTON", L"\x5b57\x5e55\x751f\x6210", WS_CHILD|WS_VISIBLE|BS_DEFPUSHBUTTON, 14, y, (W-24)/2 - 3, 36, g_wnd, (HMENU)IDC_GENERATE, g_hInst, 0); g_tabSubCtrls.push_back(hw);
    hw = CreateWindowExW(0, L"BUTTON", L"SRT\x30a8\x30af\x30b9\x30dd\x30fc\x30c8", WS_CHILD|WS_VISIBLE, 14 + (W-24)/2 + 6, y, (W-24)/2 - 3, 36, g_wnd, (HMENU)IDC_EXPORT_SRT, g_hInst, 0); g_tabSubCtrls.push_back(hw);
    y += 42;
    ...
    y += 18;
    g_status = CreateWindowExW(0, L"STATIC", L"Ready (v2.8)", WS_CHILD|WS_VISIBLE, 14, y, W-24, 20, g_wnd, 0, g_hInst, 0); g_tabSubCtrls.push_back(g_status);

    // ================================================================
    // Tab 1: 設定 (Settings) — Backend, Beam/Temp, Device, Layer, etc.
    // ================================================================
    y = tabY + 8;
    hw = CreateWindowExW(0, L"STATIC", L"Backend:", WS_CHILD, 14, y+3, 52, 18, g_wnd, 0, g_hInst, 0); g_tabSettingsCtrls.push_back(hw);
    g_backendCombo = CreateWindowExW(0, L"COMBOBOX", L"", WS_CHILD|CBS_DROPDOWNLIST, 70, y, 130, 80, g_wnd, 0, g_hInst, 0); g_tabSettingsCtrls.push_back(g_backendCombo);
    ...
    hw = CreateWindowExW(0, L"STATIC", L"Device:", WS_CHILD, 210, y+3, 44, 18, g_wnd, 0, g_hInst, 0); g_tabSettingsCtrls.push_back(hw);
    g_deviceCombo = CreateWindowExW(0, L"COMBOBOX", L"", WS_CHILD|CBS_DROPDOWNLIST, 256, y, 84, 80, g_wnd, 0, g_hInst, 0); g_tabSettingsCtrls.push_back(g_deviceCombo);
    ...
    y += 26;
    hw = CreateWindowExW(0, L"STATIC", L"Beam:", WS_CHILD, 14, y+3, 38, 18, g_wnd, 0, g_hInst, 0); g_tabSettingsCtrls.push_back(hw);
    g_qualityEdit = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"5", WS_CHILD|ES_NUMBER|ES_CENTER, 52, y, 30, 22, g_wnd, 0, g_hInst, 0); g_tabSettingsCtrls.push_back(g_qualityEdit);
    hw = CreateWindowExW(0, L"STATIC", L"(1-10)", WS_CHILD, 85, y+3, 42, 18, g_wnd, 0, g_hInst, 0); g_tabSettingsCtrls.push_back(hw);
    hw = CreateWindowExW(0, L"STATIC", L"Temp:", WS_CHILD, 135, y+3, 38, 18, g_wnd, 0, g_hInst, 0); g_tabSettingsCtrls.push_back(hw);
    g_tempEdit = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"0", WS_CHILD|ES_CENTER, 175, y, 30, 22, g_wnd, 0, g_hInst, 0); g_tabSettingsCtrls.push_back(g_tempEdit);
    hw = CreateWindowExW(0, L"STATIC", L"(0=\x56fa\x5b9a)", WS_CHILD, 208, y+3, 80, 18, g_wnd, 0, g_hInst, 0); g_tabSettingsCtrls.push_back(hw);
    y += 26;
    hw = CreateWindowExW(0, L"STATIC", L"Layer:", WS_CHILD, 14, y+3, 40, 18, g_wnd, 0, g_hInst, 0); g_tabSettingsCtrls.push_back(hw);
    g_layerEdit = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"2", WS_CHILD|ES_NUMBER|ES_CENTER, 56, y, 36, 22, g_wnd, 0, g_hInst, 0); g_tabSettingsCtrls.push_back(g_layerEdit);
    hw = CreateWindowExW(0, L"STATIC", L"(\x5b57\x5e55\x914d\x7f6e\x5148)", WS_CHILD, 96, y+3, 80, 18, g_wnd, 0, g_hInst, 0); g_tabSettingsCtrls.push_back(hw);
    hw = CreateWindowExW(0, L"STATIC", L"\x6587\x5b57\x6570:", WS_CHILD, 185, y+3, 48, 18, g_wnd, 0, g_hInst, 0); g_tabSettingsCtrls.push_back(hw);
    g_maxCharEdit = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"0", WS_CHILD|ES_NUMBER|ES_CENTER, 235, y, 36, 22, g_wnd, 0, g_hInst, 0); g_tabSettingsCtrls.push_back(g_maxCharEdit);
    hw = CreateWindowExW(0, L"STATIC", L"(0=\x7121\x5236\x9650)", WS_CHILD, 275, y+3, 65, 18, g_wnd, 0, g_hInst, 0); g_tabSettingsCtrls.push_back(hw);
    y += 28;
    hw = CreateWindowExW(0, L"STATIC", L"\x66f8\x5f0f:", WS_CHILD, 14, y+3, 42, 18, g_wnd, 0, g_hInst, 0); g_tabSettingsCtrls.push_back(hw);
    g_templateLabel = CreateWindowExW(0, L"STATIC", L"\x30c7\x30d5\x30a9\x30eb\x30c8", WS_CHILD, 58, y+3, 110, 18, g_wnd, 0, g_hInst, 0); g_tabSettingsCtrls.push_back(g_templateLabel);
    hw = CreateWindowExW(0, L"BUTTON", L"\x9078\x629e", WS_CHILD, 175, y, 55, 22, g_wnd, (HMENU)IDC_TEMPLATE, g_hInst, 0); g_tabSettingsCtrls.push_back(hw);
    hw = CreateWindowExW(0, L"BUTTON", L"\x30ea\x30bb\x30c3\x30c8", WS_CHILD, W-75, y, 65, 22, g_wnd, (HMENU)IDC_RESET_TPL, g_hInst, 0); g_tabSettingsCtrls.push_back(hw);
    y += 28;
    hw = CreateWindowExW(0, L"STATIC", L"\x5b57\x5e55\x5ef6\x9577:", WS_CHILD, 14, y+3, 62, 18, g_wnd, 0, g_hInst, 0); g_tabSettingsCtrls.push_back(hw);
    g_lingerEdit = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"1.0", WS_CHILD|ES_CENTER, 78, y, 40, 22, g_wnd, 0, g_hInst, 0); g_tabSettingsCtrls.push_back(g_lingerEdit);
    hw = CreateWindowExW(0, L"STATIC", L"\x79d2 (0=\x306a\x3057)", WS_CHILD, 122, y+3, 100, 18, g_wnd, 0, g_hInst, 0); g_tabSettingsCtrls.push_back(hw);
    y += 28;
    hw = CreateWindowExW(0, L"STATIC", L"\x2500\x2500 \x30c6\x30ad\x30b9\x30c8\x51e6\x7406 \x2500\x2500", WS_CHILD, 14, y, W-24, 18, g_wnd, 0, g_hInst, 0); g_tabSettingsCtrls.push_back(hw);
    y += 22;
    g_chkRemovePunct = CreateWindowExW(0, L"BUTTON", L"\x53e5\x8aad\x70b9\x524a\x9664", WS_CHILD|BS_AUTOCHECKBOX, 14, y, 100, 18, g_wnd, 0, g_hInst, 0); g_tabSettingsCtrls.push_back(g_chkRemovePunct);
    g_chkRemoveExclam = CreateWindowExW(0, L"BUTTON", L"!?\x524a\x9664", WS_CHILD|BS_AUTOCHECKBOX, 116, y, 72, 18, g_wnd, 0, g_hInst, 0); g_tabSettingsCtrls.push_back(g_chkRemoveExclam);
    g_chkNormalize = CreateWindowExW(0, L"BUTTON", L"\x5168\x534a\x89d2\x6b63\x898f\x5316", WS_CHILD|BS_AUTOCHECKBOX, 192, y, 120, 18, g_wnd, 0, g_hInst, 0); g_tabSettingsCtrls.push_back(g_chkNormalize);

    // ================================================================
    // Tab 2: 精度 (Accuracy / Speed)
    // ================================================================
    y = tabY + 8;
    g_chkNoPrevText = CreateWindowExW(0, L"BUTTON", L"\x5e7b\x8074\x9632\x6b62", WS_CHILD|BS_AUTOCHECKBOX, 14, y, 82, 18, g_wnd, 0, g_hInst, 0); g_tabAccCtrls.push_back(g_chkNoPrevText);
    SendMessageA(g_chkNoPrevText, BM_SETCHECK, BST_CHECKED, 0);
    g_chkRepPenalty = CreateWindowExW(0, L"BUTTON", L"\x7e70\x8fd4\x3057\x6291\x5236", WS_CHILD|BS_AUTOCHECKBOX, 100, y, 90, 18, g_wnd, 0, g_hInst, 0); g_tabAccCtrls.push_back(g_chkRepPenalty);
    SendMessageA(g_chkRepPenalty, BM_SETCHECK, BST_CHECKED, 0);
    g_chkWordTs = CreateWindowExW(0, L"BUTTON", L"\x5358\x8a9eTS", WS_CHILD|BS_AUTOCHECKBOX, 196, y, 66, 18, g_wnd, 0, g_hInst, 0); g_tabAccCtrls.push_back(g_chkWordTs);
    g_chkBatched = CreateWindowExW(0, L"BUTTON", L"Batched", WS_CHILD|BS_AUTOCHECKBOX, 268, y, 70, 18, g_wnd, 0, g_hInst, 0); g_tabAccCtrls.push_back(g_chkBatched);
    y += 24;
    hw = CreateWindowExW(0, L"STATIC", L"\x30d2\x30f3\x30c8\x6587: (\x56fa\x6709\x540d\x8a5e\x3084\x5185\x5bb9\x3092\x5165\x529b)", WS_CHILD, 14, y, W-24, 18, g_wnd, 0, g_hInst, 0); g_tabAccCtrls.push_back(hw);
    y += 20;
    g_promptEdit = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"", WS_CHILD|ES_MULTILINE|ES_AUTOVSCROLL|WS_VSCROLL, 14, y, W-24, 50, g_wnd, 0, g_hInst, 0); g_tabAccCtrls.push_back(g_promptEdit);
    y += 54;
    hw = CreateWindowExW(0, L"STATIC", L"\x30db\x30c3\x30c8\x30ef\x30fc\x30c9: (\x30ab\x30f3\x30de\x533a\x5207\x308a)", WS_CHILD, 14, y, W-24, 18, g_wnd, 0, g_hInst, 0); g_tabAccCtrls.push_back(hw);
    y += 20;
    g_hotwordsEdit = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"", WS_CHILD|ES_AUTOHSCROLL, 14, y, W-24, 22, g_wnd, 0, g_hInst, 0); g_tabAccCtrls.push_back(g_hotwordsEdit);

    // ================================================================
    // Tab 3: 環境 (Setup) — ffmpeg, Python, whisper paths
    // ================================================================
    y = tabY + 8;
...
if(MSVC)
    target_compile_options(whisper_subtitle PRIVATE /utf-8 /wd4828)
endif()
...
try {
    ...
    if($aux2){
        ...
        Write-Host " BUILD SUCCESS! v2.8" -ForegroundColor Green
        Write-Host "============================================" -ForegroundColor Green
        Write-Host "Output: $dest" -ForegroundColor Yellow
        Write-Host ""
        Write-Host "v2.8 Features:" -ForegroundColor Cyan
        Write-Host "  + SDK updated to beta40 (EDIT_SECTION/HOST_APP_TABLE)" -ForegroundColor White
        Write-Host "  + Build: /utf-8 /wd4828 compile options" -ForegroundColor White
        Write-Host "  + Fixed source encoding (UTF-8 without BOM)" -ForegroundColor White
        Write-Host "  + Fixed build log mojibake (Console OutputEncoding)" -ForegroundColor White
        Write-Host "  + Segment quality filter / Hallucination threshold" -ForegroundColor White
        Write-Host "  + Hotwords / kotoba-whisper / Batched inference" -ForegroundColor White
