...
| **Batched** | チェックボックス | OFF | GPU 並列処理を有効化。CUDA 環境でのみ動作 |
| **VAD無音カット** | チェックボックス | **OFF** | 無音区間を除いてから認識する。**faster-whisper 専用**。詳細は下記 |
| ⚠️⚠️**ヒント文** | テキスト入力 | 空 | initial_prompt に渡す文章。固有名詞や専門用語を入れると認識精度が向上　現状、ここに日本語を入れるとエラーが起きます。原因を調査中です。 |
| ⚠️⚠️**ホットワード** | テキスト入力 | 空 | カンマ区切りで指定。デコーダで特定の単語をブースト　現状、ここに日本語を入れるとエラーが起きます。原因を調査中です。 |　
...
* **ヒント文 / ホットワード**: 固有名詞や専門用語の認識精度を向上　⚠️⚠️修正中
