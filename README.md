...
| Python | **3.10 以上**（3.13 / 3.14 も可） | [ダウンロード](https://www.python.org/downloads/)（**python.org 版**） |
...
* **ハルシネーション対策**: 前文無効化・品質フィルタ・無音区間閾値
* **温度自動フォールバック**: Temp=0 で段階的に温度を上げて再試行
...
* 誰も話していない場所に出る「ご視聴ありがとうございました」等の定型句を、除去できるようにしました（**こちらは意図的に減らす側の変更です**。誤って消される場合は無効化できます → [設定ファイル](#自分で置ける設定ファイル任意どちらも無くて構いません)）
