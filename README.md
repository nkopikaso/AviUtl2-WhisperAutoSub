...
2. 「初期設定」タブを開く
3. Python と ffmpeg のパスが表示されていることを確認（自動検出されます）
4. **「セットアップ」ボタン** をクリック → バックエンドとモデルが自動でインストールされます

> 初回セットアップでは数GB程度のダウンロードが発生します。回線速度によっては数分〜十数分かかります。
...
- **自動インストール**: セットアップボタンでバックエンド・モデルを一括導入
...
- 初回実行時はモデルダウンロードのため時間がかかります（large-v3で約3GB）
