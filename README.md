...
**v2.9.36（最新）は「字幕が消える・間違ったものが出る」不具合をまとめて修正しました。**
v2.9.26 以前をお使いの方は更新をおすすめします。

心当たりがあれば、そのまま直っている可能性があります。

* 動画の**冒頭の字幕が出ない**（「先行表示」を使っている場合）
* 生成し直したら**前の動画の字幕**が並んだ
* **書式テンプレート**を指定したら、字幕が消えて別のオブジェクトが並んだ
* ときどき**字幕が抜ける**（kotoba-whisper をお使いの場合は常に該当）
* faster-whisper なのに **GPU が使われず遅い**

生成後の画面に注意書きが出るようになりました（これまで表示されていませんでした）。
