...
**v2.9.26 は「GPU があるのに使われない」問題を 2 件修正しました。**
心当たりのある方（生成が思ったより遅い方）は更新をおすすめします。

それ以前の **v2.9.25 では「字幕が消える・減る」を修正**しています。
「ときどき字幕が抜ける」「置いたはずのオブジェクトが消えた」といった症状は
独立した複数の原因によるもので、そこで解消しました（詳細は[更新履歴](#更新履歴)）。
...
* **GPU があるのに CPU で動いているとき、その理由を表示するようにしました**

以前のバージョンで導入した方は、GPU 用のライブラリ（cuBLAS / cuDNN）が入っていない状態に
なっている場合があります。**エラーは出ないまま CPU で動く**ため、
「なんとなく遅い」としか分かりませんでした。

生成後に「CUDA未導入でCPU実行。環境タブで再セットアップ」と表示します。
表示が出たら「環境」タブでセットアップを実行してください（v2.9.35 以降で正しく導入されます）。

なお **字幕生成は止めません**。CPU でも問題なく動作します。

### v2.9.35（2026-07-31）

* **faster-whisper で GPU が使われないまま直せなくなる場合があったのを修正**

faster-whisper が GPU を使うには専用のライブラリ（cuBLAS / cuDNN）が必要ですが、
その導入処理が **「faster-whisper は導入済み」と判定されると飛ばされる**位置にありました。
別の場所に Python を入れている場合などにこの状態になり、
**セットアップを何度実行しても GPU が有効になりません**でした。

v2.9.28 以降はこの状態でも落ちずに CPU で動作するため、
「エラーは出ないのに遅い」という気づきにくい形になっていました。

導入処理を独立させ、必要なときだけ確実に実行するようにしました
（既にライブラリがある場合は約 1GB のダウンロードを行いません）。

### v2.9.34（2026-07-31）

* **字幕生成に失敗したあと、SRT エクスポートが前回の字幕を書き出すことがあったのを修正**

ffmpeg や Python が見つからずに生成が中止された場合、前回の字幕データが残ったままになり、
そのあと SRT を書き出すと**前回の内容**が出ていました。
生成を始めた時点で前回のデータを破棄するようにしました。

### v2.9.33（2026-07-31）

* **`hallucination_phrases.txt` を Shift-JIS で保存すると、内容が無視されていたのを修正**

メモ帳などで「ANSI」形式で保存すると読み込みに失敗し、**編集した内容が反映されないまま**
既定の 12 語が使われていました。UTF-8 と Shift-JIS のどちらでも読めるようにしました。

このファイルは定型句フィルタを止めるための唯一の手段なので、
効かないことに気づけないのが問題でした。

### v2.9.32（2026-07-31）

* **一部の字幕が黙って消えることがあったのを修正しました**

単語ごとの時刻が取れない場合（**kotoba-whisper では常にこちら**）、直前の字幕と
表示時間が重なると、その字幕が**出力されずに消えて**いました。
重ならないように詰めて、必ず表示するようにしました。

「文節区切り」で細かく区切っているときに起こりやすい状態でした。

### v2.9.31（2026-07-31）

**SRT エクスポートの不具合を 2 件修正しました。**

* **2 行の字幕が、SRT では `前\n後` という文字列になっていたのを修正**
  「最大 2 行にまとめる」や「文節区切り」を使うと、SRT の中で改行にならず
  `\n` という 2 文字がそのまま表示されていました。正しく改行するようにしました。
* **「先行表示」を使うと、長さが 0 の字幕が SRT に混ざることがあったのを修正**

字幕生成そのものには影響しません。SRT ファイルの中身だけが変わります。

### v2.9.30（2026-07-31）

* **生成後に出るはずの注意書きが、画面に表示されていなかった問題を修正しました**

「Done! ◯個の字幕を配置」を表示する欄が 1 行しかなく、そのうしろに付けていた注意書きが
**右端で切れて見えていませんでした**。欄を 3 行に広げ、注意書きは行を分けて表示します。

これにより、これまで気づけなかった以下の注意書きが表示されるようになります。

* 既存オブジェクトと重なって配置できなかった字幕がある（v2.9.21 で追加、以来ずっと不可視）
* 文節区切りが使えなかった（fugashi 未導入）
* 再生速度が 100% でないクリップがある
...
不具合のご報告をいただいたときに、こちらで原因を絞り込みやすくするための変更です。

### v2.9.28（2026-07-31）

* **再生速度を変更したクリップで、字幕の位置がずれることをお知らせするようにしました**

本プラグインは**再生速度に対応していません**。100% 以外に設定したクリップでは、
音声だけが伸び縮みして字幕は動かないため、**クリップの後ろにいくほど字幕がずれます**
（後半が欠けることもあります）。これまでは何の表示も出ませんでした。

生成後のメッセージに「◯個のクリップの再生速度が100%でないため字幕の位置がずれます」と
表示します。**動作そのものはこれまでと同じ**で、表示が増えただけです。

> 現時点での回避策: 速度を変更したクリップは、いったん 100% に戻して字幕を生成し、
> そのあとで速度を戻してください。再生速度への対応は今後の課題です。

### v2.9.27（2026-07-30）

**「最大 2 行にまとめる」がチェックしても効かない場合があったのを修正しました。**

* **文字数制限を 0（無制限）にしていると、2 行になりませんでした**
  内部で「分割しない」と判断され、2 行にまとめる対象が生まれていませんでした。
  文字数制限が 0 のときは 20 文字を目安に分割します（「文節区切り」を使っている場合は
  以前から同じ動作でした。経路によって挙動が違っていたのを揃えています）。

* **「短い字幕を結合」と同時に使うと、2 行になりませんでした**
  結合の上限が 1 行ぶんのままだったため、2 行に分かれるだけの長さになりませんでした。
  2 行を許可しているときは 2 行ぶんまで結合します。

いずれも **「最大 2 行にまとめる」が OFF の場合は、これまでと完全に同じ動作**です。
