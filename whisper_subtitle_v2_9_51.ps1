##############################################################################
# Whisper Subtitle Plugin for AviUtl2 - v2.9.51
...
#
# ビルド: .\whisper_subtitle_v2_9_51.ps1 [出力ディレクトリ]
# 要件: Visual Studio 2022, CMake 3.15+
# 出力: whisper_subtitle_v2_9_51.aux2 (既存の whisper_subtitle_v2_8_10.aux2 / whisper_subtitle.aux2 は上書きしない)
#
...
$projDir = "$d\aviutl2_dev\whisper_subtitle_plugin_v2_9_51"
$src = "$projDir\src"
...
$logFile = "$logDir\whisper_build_log_v2_9_51.txt"
$ErrorActionPreference = "Continue"
[Console]::OutputEncoding = [System.Text.Encoding]::UTF8

"" | Out-File $logFile -Encoding UTF8
"Whisper Subtitle v2.9.51 Build $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')" | Out-File $logFile -Append -Encoding UTF8

Write-Host ""
Write-Host "============================================" -ForegroundColor Cyan
Write-Host " Whisper Subtitle v2.9.51 - Build" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""
    ...
    for ci, clip in enumerate(clips):
        ...
        try:
            ...
            for seg in result.get("segments", []):
                ...
                # (短い相づちや固有名詞などの、正しい認識結果)。一方、本物のゴミは -5.865。
...
static void SetupThread(){
    ...
    if(python.empty()){
        ...
        SetStatusW(L"Ready (v2.9.51)"); // v2.8.10【I】: SetWindowTextW直接呼びを置き換え(環境タブ凍結対策)+バージョン更新
        SetProgress(0);
    ...
    SetStatusW(L"Ready (v2.9.51)"); // v2.8.10【I】: SetupThread正常終了時にg_statusSetupが取り残され「fugashi確認中...」等で凍結していたバグの本丸修正+バージョン更新
    SetProgress(0);
...
static bool RunFasterWhisper(int mi, int di, int bi, int beamSize, int li, float temp, bool noPrevText, bool wordTs, bool repPenalty, const std::string& promptText, const std::string& hotwordsText, bool batched, bool vadFilter){ // v2.9.5: vadFilter追加
            ...
            if(!needAutoInstall){
                ...
                if(pyOut.find("Numpy is not available") != std::string::npos
                   ...
                   || line.find("Numpy is not available") != std::string::npos){
                    em += "\n\n\n\x28\x50\x79\x54\x6f\x72\x63\x68\x20\xe3\x81\xaf\xe7\xb4\x84\x34\x2e\x33\x47\x42\xe3\x81\x82\xe3\x82\x8a\xe3\x80\x81\xe6\x99\x82\xe9\x96\x93\xe3\x81\x8c\xe3\x81\x8b\xe3\x81\x8b\xe3\x82\x8a\xe3\x81\xbe\xe3\x81\x99\x29\n\n";
                }
                em += line.substr(6);
    ...
    if(needAutoInstall && !installPkg.empty()){
        ...
        bool pyOk2 = RunProcess(wCmd, pyOut2, tmo);
...
static void TranscribeThread(){
    ...
    {std::ofstream f(Utf8ToWide(GetPluginDir() + "\\whisper_debug.log"), std::ios::trunc); f << "=== Whisper Subtitle v2.9.51 ===\n";}
    ...
    if(ffmpeg.empty()){
        MsgBox(g_wnd,
            ...
            "Error", MB_OK|MB_ICONERROR);
        SetStatus("Ready (v2.9.51)"); SetProgress(0); SetBusy(false); return;
    }
    std::string python = GetEffectivePython();
    if(python.empty()){
        MsgBox(g_wnd,
...
static void WriteTestLog(){
    ...
    f << "===== Whisper Subtitle v2.9.51 test log =====\n";
    f << "datetime  : " << dbuf << "\n";
    ...
    if(m == WM_COMMAND){
        ...
        if(HIWORD(w) == BN_CLICKED){
            ...
            if(id == IDC_GENERATE){
                ...
                if(!p.empty()){
                    if(LoadTemplate(p)){ g_templatePath = p; UpdateTemplateLabel(); SaveSettings(); }
                    // v2.9.27【R1】黙って無視すると「選べたのに効かない」になる。理由を出す。
                        ...
                        "\xe3\x81\xa7\xe4\xbd\x9c\xe3\x81\xa3\xe3\x81\x9f .object \xe3\x82\x92\xe9\x81\xb8\xe3\x82\x93\xe3\x81\xa7\xe3\x81\x8f\xe3\x81\xa0\xe3\x81\x95\xe3\x81\x84\xe3\x80\x82",
...
extern "C" __declspec(dllexport) void __cdecl RegisterPlugin(HOST_APP_TABLE* host){
    ...
    g_status = CreateWindowExW(0, L"STATIC", L"Ready (v2.9.51)", WS_CHILD|WS_VISIBLE, SC(14), SC(y), SC(W-24), SC(54), g_wnd, 0, g_hInst, 0); g_tabSubCtrls.push_back(g_status);
    ...
    y += 18; // y=330
    g_statusSetup = CreateWindowExW(0, L"STATIC", L"Ready (v2.9.51)", WS_CHILD, SC(14), SC(y), SC(W-24), SC(20), g_wnd, 0, g_hInst, 0); g_tabSetupCtrls.push_back(g_statusSetup);
...
project(whisper_subtitle_v2_9_51 LANGUAGES CXX)
set(CMAKE_CXX_STANDARD 17)
add_library(whisper_subtitle_v2_9_51 SHARED src/whisper_subtitle.cpp)
target_compile_definitions(whisper_subtitle_v2_9_51 PRIVATE UNICODE _UNICODE)
if(MSVC)
    target_compile_options(whisper_subtitle_v2_9_51 PRIVATE /utf-8 /wd4828)
endif()
...
set_property(TARGET whisper_subtitle_v2_9_51 PROPERTY MSVC_RUNTIME_LIBRARY "MultiThreaded")
set_target_properties(whisper_subtitle_v2_9_51 PROPERTIES
    OUTPUT_NAME "whisper_subtitle_v2_9_51"
    SUFFIX ".aux2"
    PREFIX ""
    RUNTIME_OUTPUT_DIRECTORY_RELEASE "`${CMAKE_BINARY_DIR}/Release"
)
target_link_libraries(whisper_subtitle_v2_9_51 PRIVATE comctl32 shell32 comdlg32 ole32)
"@
...
try {
    ...
    # Find output
    $aux2 = Get-ChildItem -Path $buildDir -Recurse -Filter "whisper_subtitle_v2_9_51.aux2" | Select-Object -First 1
    if($aux2){
        ...
        $dest = Join-Path $destDir "whisper_subtitle_v2_9_51.aux2"
        Copy-Item $aux2.FullName $dest -Force
        Write-Host ""
        Write-Host "============================================" -ForegroundColor Green
        Write-Host " BUILD SUCCESS! v2.9.51" -ForegroundColor Green
        Write-Host "============================================" -ForegroundColor Green
