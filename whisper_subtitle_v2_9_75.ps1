...
# 実測(2026-08-24 実機/89件中2件)。いずれも文末記号のあとに空白が残っていた。
...
def _morph_group(text, maxc):
    ...
    for w in tg(text):
        ...
        #   例)「きょうは いい天気」「ちょっと まってね」のように和字の間に空白が入る
    ...
    #   17字/maxc14: 「そろそろ帰りたくなって」(11) + 「きたけど」(4)
    # 人は均等に割る:「そろそろ」(4) + 「帰りたくなってきたけど」(11)
...
# 実測(開発者素材): ある区間の直前で 6.470-6.850 の 380ms の発話が
...
#     別の区間が 4.780→3.780 と1秒早まって悪化した(立ち上がりまで 280→1280ms)。
...
def _reclaim_orphan(t, prev_end, rr, env):
    ...
    #  (これが無いと、立ち上がり6ms手前の区間まで動かしてしまった)。
...
#   例: ある行が13文字に 0.583秒 = **秒速22文字**。人間には不可能。
#   同じセグメントの次の行は 2.534s に置かれたが、whisper 自身の単語時刻(DTW)は 3.000s。
#   開発者の指摘(前の行は遅い / 次の行は早すぎる)と両方一致した。
    ...
    for ci, clip in enumerate(clips):
        ...
        try:
            ...
            for _si, seg in enumerate(segments):
                ...
                # **後ろへ押す**(実測: ある区間を 7.240s→7.650s と410ms遅らせていた)。
